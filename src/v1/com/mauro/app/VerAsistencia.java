...
public class VerAsistencia extends javax.swing.JFrame {
    ...
    public VerAsistencia() {
        initComponents();
        int sum = 0;
        ArrayList<Asistencia> reg = Principal.colegio.cursos.get(Principal.seleccion)
                .alumnos.get(Principal.numAlumno).registro;
        for (Asistencia asist: reg) {
            String[] f = asist.getFecha();
            String desc = f[0] + "/" + f[1] + "/" + f[2] + ": " + (asist.getPresente()
                    ? "Presente" : "Ausente");
        ...
        porcentaje.setText(Integer.toString((sum * 100) / reg.size()) + "%");
    }
