...
import v1.com.mauro.librodeclases.Asistencia;
...
public class NuevaAsistencia extends javax.swing.JFrame {
    ...
    private void listo(boolean crea) {
        if (crea) {
            Asistencia asist;
            String f = String.format("%02d-%02d-%d", diaAsistencia.getValue(),
                    mesAsistencia.getValue(), agnoAsistencia.getValue());
            try {
                asist = new Asistencia(f, presente.isSelected());
                Principal.colegio.cursos.get(Principal.seleccion).alumnos.get(
                        Principal.numAlumno).registro.add(asist);
